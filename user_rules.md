...
- **完整自動接力預授權例外 (Full Automatic Handoff Pre-Authorization) [HARD]**：當使用者明確輸入「啟用完整自動接力」、「繼續完整自動接力」、「恢復完整自動接力」或等效語句，且存在可驗證的 Task Contract / handoff / machine state 時，該指令視為對 Task Contract 內**可逆、task-owned、已授權、非 production、非破壞性**連續工程動作的一次性明確授權。此模式的目的就是持續執行到完成或真正 hard blocker，不得被一般逐步確認規則切斷。
  ...
  - **完整自動接力監控拓樸自動確保 [HARD]**：平台支援排程／輪詢時，使用者啟用或恢復 `FULL_AUTOMATIC_HANDOFF` 即同時預授權建立、更新、重啟與去重該工作流的安全監控拓樸，不需逐一再次詢問。全域恰好一個唯讀 Full-Auto Monitor + 一個 Recovery Supervisor；每個 current active `taskRole=execution` durable task 恰好一個綁 exact `taskId + executionProjectRef` 的 Execution Watch 與一個永遠 read-only 的 Status Pulse。Status Pulse 不得 workflow mutation／writer lease／改檔／commit/push／takeover；Global Monitor 不得直接做 product mutation。既有相同 task/role 排程停用時優先重新啟用，禁止建立 duplicate。`HARD_BLOCKED` 至少保留 Pulse；`USER_PAUSED` 停 Execution Watch、保留 Pulse；`DONE / USER_CANCELLED` 停該 Task 的 Watch + Pulse但保留歷史 evidence。Recovery Supervisor 只能 project-scoped 救援，不得成為第二個 product Executor或跨越 production/release/secret hard gate。
