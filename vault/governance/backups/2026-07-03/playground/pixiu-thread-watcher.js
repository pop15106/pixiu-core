...
function run(rawInput, options = {}) {
  ...
  const stateDir = options.stateDir || path.join(corePath, 'vault', 'memory', 'hook-state', 'codex-thread-watcher');
...
function runSessionEndAutoRecap({ input, entries, state, corePath, tempDir, transcriptPath, sessionId }) {
  ...
  const normalizedPath = path.join(tempDir, `${safeFileName(sessionId)}-${hash.slice(0, 12)}.jsonl`);
