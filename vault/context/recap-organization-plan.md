...
status: superseded
summary: 原草案主張先不搬 recap 原件；2026-06-08 已被新規則取代，recap 原件改為依專案與月份存放。
...
本文件保留為歷史草案；其中「不搬動既有 recap 原件」已被 2026-06-08 使用者授權的新規則取代。
...
與專案無關、屬於 PixiuCore 母體治理、AI 行為、skill/workflow、vault 結構、recap 規範或跨 AI 操作準則者，檔名前綴與第一層資料夾使用 `母體`；frontmatter `project` 仍使用 canonical key `PIXIUCORE`。

檔名只保留日期，不加入時分秒；同日同專案撞名時，改用更精準內容或非時間性短識別詞。
