...
topic: manual-codex-scoring
status: done
tags: [recap, session, pixiucore, auto-research, manual-scoring]
summary: 設計 Auto Research 在無 API 情境下的 Manual Codex Scoring 流程與評分落點。
---
...
API 預留策略：
- api/ 目錄保留 .gitkeep，第一版不實作。
- candidate 參數中以 api_client: TODO_NO_API 標記。
