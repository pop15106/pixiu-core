...
summary: 更新 PixiuCore 在 gravityTest 的 README，補齊母體初始化、vault 與使用規則說明。
---
...
1. 依 PixiuCore 啟動規則讀取母體 context：vault/README.md、user_rules.md、founder-profile.md、agent-persona.md、memory-summary.md，並因目前工作目錄為 PCLMS，也讀取 pclms-overview.md 與 tech-stack.md。
2. 盤點 <workspace-root>\pixiu-core 實際內容，確認頂層目錄包含 .agent/、.agents/、.codex/、.cursor/、.opencode/、agents/、commands/、skills/、Tools/、vault/ 等。
...
- 目前步驟：README 更新已完成，正在依使用者 recap 要求回寫母體記憶。
- 整體進度：1 / 1 Phase 完成。
- 各 Phase 狀態：
...
- [ ] 檢查 pack-for-friend 是否排除 vault/identity、vault/memory 與大型 Backup/。
