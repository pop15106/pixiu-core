...
summary: 盤點 PCLMS_BK 的 L1、L4、N1C 訊息傳送規則與前置條件，保留待續任務版本。
---
...
<!-- AI_INBOX_START -->
- [ ] 修正 `EnqueueN1C_modified.java` 中的冗餘 `moveFolder` 呼叫以消除誤報。
- [ ] 修正 `EnqueueN1C_modified.java` 的 `getRecvId` SQL，加入監管編號過濾。
- [ ] 驗證資料庫 `WAREHSE` 的 `SEPID` 是否已補全。
- [ ] 測試新 Port 的網路連通性。
<!-- AI_INBOX_END -->
