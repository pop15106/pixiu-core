...
system: PIXIUCORE
repo: Playground
topic: Pixiu-Auto-Research-Manual-Codex-Scoring
status: accepted
decision: Pixiu Auto Research Manual Codex Scoring Mode
alternative: 現在就實作 LLM API client：目前沒有 API，且會增加安全與設定負擔。 直接做全自動 agent loop：尚未有穩定 evaluator 與 registry，容易變成不可追蹤的自動化。 只產 DOCX 不產範本：無法實際試跑手動閉環。
summary: Pixiu Auto Research Manual Codex Scoring Mode
