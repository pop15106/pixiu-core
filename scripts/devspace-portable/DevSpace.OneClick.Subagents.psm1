...
function Install-DevSpaceSubagentWindowsPatch {
    ...
    $workflowGuidanceReplacement = 'Use project_resolve, workflow_create, workflow_list, workflow_update, and workflow_takeover for durable project-scoped coordination. Project context is mandatory before progress lookup or continuation. When a workspace is open and the user asks generic progress or says continue/resume/take over, the current canonical workspace project is authoritative: call workflow_list with its default execution_only mode and never select another project because its task is newer, active, pending, cross-project visible, or globally recent. When the user is outside a project workspace and explicitly names a project, call project_resolve first, open the unique returned canonical root, then inspect that project. Missing or ambiguous project resolution must fail closed instead of guessing. A read-only lookup of another project never changes the current session execution binding; only an explicit switch or targeted cross-project handoff acknowledged by the target project may change executionProjectRef. Status Pulse, Recovery Supervisor, reviewer-watch, and governance tasks are non-implicit and must not be selected by generic progress or continuation. Treat clear natural-language continuation intent such as "continue in the next session", "another chat should take over", "I will open a new chat and continue", or an explicitly named other project taking over as a workflow request; do not require the user to say workflow, handoff, claim, or acknowledge. When the current session hands work off, create or reuse the matching task, claim it if needed, and use targetWorkspaceId for an explicit cross-project transfer. When a new session resumes or takes over, call workflow_list first and acknowledge only the single project-affine pending handoff. workflow_takeover is only for stale-owner recovery and never bypasses project affinity, review, production, release, secret, or destructive-action gates. Use same_project for another session in the same canonical project root and cross_project only when every related workspace is explicitly known and opened. Ask only when the target project, scope, or matching task is ambiguous. Never call workflow_run because of continuation intent alone; workflow_run requires separate explicit user authorization in the current conversation. Use workflow_sync only for a run the user already authorized. Use exec_command for npm, builds, tests, and DevSpace Agent status commands'
    ...
    $workflowGuidanceCandidates = @(
        ...
        'Use workflow_create, workflow_list, workflow_update, and workflow_takeover for durable cross-session coordination. Treat clear natural-language continuation intent such as "continue in the next session", "another chat should take over", "I will open a new chat and continue", or an explicitly named other project taking over as a workflow request; do not require the user to say workflow, handoff, claim, or acknowledge. When the current session is handing work off, create or reuse the matching task, claim it if needed, then hand it off. When a new session says it is resuming or taking over, call workflow_list first and acknowledge the single clearly matching pending handoff. workflow_takeover is only for stale-owner recovery: require exact expectedRevision and expectedOwner, at least 600 seconds without owner heartbeat, no active task runner, and missing/stale Execution Watch; never use it to bypass review, production, release, secret, or destructive-action gates. Use same_project for another session in the same workspace root and cross_project only when the target project workspace is explicitly known and opened. Ask only when the target, scope, or matching task is ambiguous or when multiple plausible tasks remain. Do not create a workflow from vague closure such as "先這樣" without clear continuation intent. Never call workflow_run because of continuation intent alone; workflow_run requires separate explicit user authorization in the current conversation to use an Agent/model. Use workflow_sync only for a run that the user already authorized. Use exec_command for npm, builds, tests, and DevSpace Agent status commands'
    )
