...
```
<workspace-root>\PCLMS_AP\JAVA\pclms_mvn\docs\gen_docx.py
<workspace-root>\PCLMS_AP\JAVA\pclms_mvn\docs\gen_flowcharts.py
```
...
## Reusable Requirement DOCX Tool

Use `scripts/make_requirement_docx.py` when the request is a requirement/specification document with tables.
...
`spec.json` must be UTF-8 and may contain:
...
## Windows UTF-8 Guardrails

- Do not pipe Chinese here-strings from PowerShell into `python -`; this session observed Chinese text becoming mojibake and the DOCX path becoming `FTP????????.docx`.
- Prefer creating a UTF-8 `.py` or `.json` file first, then execute it with Python.
- Keep command lines mostly ASCII and pass Chinese content through UTF-8 files.
- Use `encoding="utf-8"` when reading JSON/spec files.
- When generating DOCX, set both normal font and East Asian font to `Microsoft JhengHei`.
- Validate the result by checking the output path exists and is larger than zero bytes.


## Book-style Teaching DOCX Workflow

Use this workflow when the requested DOCX should feel like a book, textbook, exam guide, handbook, or long-form teaching reference.

Required structure:

1. **Cover page**
   - Large title
   - Subtitle that states the learning promise
   - Domain / chapter map
   - Version or year

2. **Table of contents**
   - Use a real Word TOC field, not a manually typed list.
   - After generation, open with Word automation and update fields before delivery when available.

3. **Teaching-oriented body**
   - Keep the original learning flow.
   - Prefer: concept -> why it exists -> design rule -> anti-pattern -> exam clue -> scenario question.
   - Add cross-domain judgment tables when the document is for certification prep.

4. **Validation**
   - Preferred: artifact-tool render to PNG.
   - If artifact-tool fails, test a minimal DOCX first. If the minimal DOCX also fails, treat it as a local renderer/toolchain issue.
   - Fallback on Windows: use Word COM to export PDF, then inspect with browser screenshots.
   - Do not deliver based only on file existence or file size.

Important lessons from 2026-04-27:

- Some source DOCX files contain duplicate Heading style IDs. artifact-tool may fail with `Argument_AddingDuplicateWithKey, Heading1`.
- Do not blindly rewrite `word/styles.xml` to deduplicate styles; Word may treat the result as corrupted.
- For old or style-damaged documents, rebuild into a clean `Document()` and copy paragraphs/tables semantically.
- Word TOC fields need field update before final delivery; otherwise users may see placeholder text.
- On Windows, Chrome headless may require elevated execution when launched from the sandbox.
